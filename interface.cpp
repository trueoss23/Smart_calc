...
void s21::Interface::click_one_of_many_button() {
  const QString temp_str_func = "sincotaqrlg";
  QPushButton *button = (QPushButton *)sender();
  if (nums_e_dot.contains(button->text().front())) {  //  num -
    case_num(button);
  } else if (button->text() == "x") {
    case_x();
  } else if (temp_str_func.contains(button->text().front())) {  //  func - current
    case_func(button);
  } else if (str_operations_without_sum.contains(button->text().front())) {  //  current != (+,-)
...
void s21::Interface::case_not_sum_operations(QPushButton *button) {
  if (!vision_list.empty()) {
    ...
    if ((vision_list.back() != "(") &&
        ...
        !str_sum.contains(vision_list.back())) {  //  no */ and +/
      add_str_in_label_and_list(button->text(), 0);
    }
  }
}
...
void s21::Interface::on_pushButton_clean_clicked() {
  if (!vision_list.empty()) {
    ...
    str_in_main_label = "";
    for (auto it = vision_list.begin(); it != vision_list.end(); it++) {
      str_in_main_label += (*it);
    }
